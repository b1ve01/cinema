	...
	<view>
		<view class="uni-container">
			<uni-table ref="table" :loading="loading" border stripe type="selection" emptyText="暂无更多数据"
				...
				<uni-tr v-for="(item, index) in movieDataAdminList" :key="index">
				...
				uni.request({
					...
					success:(res)=>{
						...
						Promise.all(requests).then(()=>{
							...
							let total = this.movieDataAdminList.length
							let data = this.movieDataAdminList.filter((item, index) => {
								const idx = index - (pageCurrent - 1) * pageSize
								return idx < pageSize && idx >= 0
							})
							if (value) {
								data = []
								this.movieDataAdminList.forEach(item => {
