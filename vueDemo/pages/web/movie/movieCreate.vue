<template>
	<view class="movieCreate">
		<view class="movieURL">
			<uni-file-picker class="movieURLCREATE" limit="1" :del-icon="false" disable-preview
		...
		<view class="movieInputRight">
			...
			<text>电影状态：</text>
			<uni-data-checkbox selectedColor="#f9da49" mode="button" selectedTextColor="#f9da49" v-model="movieData.movieState"
				:localdata="state" @change="changeState"></uni-data-checkbox>
			</uni-forms-item>
			
			<uni-forms-item name="movieType" class="movieType">
				<text>电影类型：</text>
				<uni-data-checkbox selectedColor="#f9da49" mode="button" selectedTextColor="#f9da49" multiple v-model="movieData.movieType" 
			...
			&& this.movieData.movieType!=''
			" class="create_movie" size="mini" @click="movie_create()">新增电影</button>
			...
			|| this.movieData.movieUrl==''
			|| this.movieData.movieType==''
			" class="create_movie_false" size="mini" >新增电影</button>

		</view>

	<view>
		<uni-popup ref="movieCreateDialog" type="dialog">
			<uni-popup-dialog :type="error" cancelText="关闭" confirmText="确定" content="是否新增该电影信息" @confirm="movieCreateConfirm"
...
<script>
	export default {
		...
		methods: {
			changeState(e) {
				console.log('e:', e);
				console.log('movieState', this.movieData.movieState);
			},
			movie_create(){
